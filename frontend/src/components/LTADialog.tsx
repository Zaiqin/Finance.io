...
const LTADialog: React.FC<LTADialogProps> = ({ open, onClose, onSubmit, nightMode }) => {
    ...
    useEffect(() => {
        ...
        if (formData.transportType === 'MRT') {
            ...
            if (lastTrip && lastTrip.formData && formData.startStation !== "" && lastTrip.formData.endStation === formData.startStation && lastTrip.formData.transportType === formData.transportType) {
            ...
            const lastTrip = trips[trips.length - 1];
            if (lastTrip && lastTrip.formData) {
                const isSameBusNumber = formData.busNumber !== "" && lastTrip.formData.busNumber === formData.busNumber;
                const isRelatedBusNumber = lastTrip.formData.busNumber.replace(/\D/g, '') === formData.busNumber.replace(/\D/g, '');
        ...
        <>
            <div className={`fixed inset-0 flex items-center justify-center ${nightMode ? 'bg-gray-900' : 'bg-black'} bg-opacity-70 z-20 p-6 ${open ? 'block' : 'hidden'}`}>
                <div className={`${nightMode ? 'bg-gray-800' : 'bg-white'} p-6 rounded-lg shadow-lg w-full max-w-md relative`}>
                    <h2 className={`text-2xl font-semibold ${nightMode ? 'text-gray-200' : 'text-gray-800'} mb-2`}>Add Public Transport Journey</h2>
                    ...
                    {trips.length > 0 ? (<h2 className={`text-xl font-bold mb-2 ${nightMode ? 'text-gray-200' : 'text-gray-800'} mt-2`}>Total Fare: ${(totalFare ?? 0).toFixed(2)}</h2>) :
                                <h2 className={`text-xl font-bold ${nightMode ? 'text-gray-200' : 'text-gray-800'} mb-2 mt-2`}>Fare: ${(fare ? fare.toFixed(2) : ' - ')}</h2>}
                            <button
                                type="button"
                                onClick={handleAddTrip}
                                className={`bg-yellow-500 hover:bg-yellow-700 text-white font-bold mt-2 py-2 px-4 rounded focus:outline-none focus:shadow-outline transition-opacity duration-300 ${(error !== null || isFetching || formData.transportType === '' || createDescription().includes('undefined')) ? 'opacity-50 cursor-not-allowed' : 'opacity-100'}`}
                                disabled={error !== null || isFetching || formData.transportType === '' || createDescription().includes('undefined')}
                            >
                            ...
                            <div className="flex items-center justify-between mt-4">
                                ...
                                <button
                                    type="button"
                                    disabled={error !== null || ((trips.length == 0 && (isFetching || fare == 0)) || (trips.length > 0 && totalFare == 0))}
                                    className={`bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline transition-opacity duration-300 ${(error !== null || ((trips.length == 0 && (isFetching || fare == 0)) || (trips.length > 0 && totalFare == 0))) ? 'opacity-50 cursor-not-allowed' : 'opacity-100'}`}
